            ...
            <tr>
                <td colspan="5" class="actions">
                    ...
                    <?php if (  ! apply_filters( 'bookworm_show_coupon_cart_totals', true ) && wc_coupons_enabled() ) { ?>
                        <div class="coupon">
                            <label for="coupon_code"><?php esc_html_e( 'Coupon:', 'bookworm' ); ?></label> <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_html__( 'Coupon code', 'bookworm' ); ?>" /> <button type="submit" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="apply_coupon" value="<?php esc_html__( 'Apply coupon', 'bookworm' ); ?>"><?php esc_html__( 'Apply coupon', 'bookworm' ); ?></button>
                            <?php do_action( 'woocommerce_cart_coupon' ); ?>
                        </div>
                    <?php } ?>

                    <button type="submit" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'bookworm' ); ?>"><?php esc_html_e( 'Update cart', 'bookworm' ); ?></button>
