...
if ( ! function_exists( 'bookworm_site_header_icons_links' ) ) {
    ...
    function bookworm_site_header_icons_links( $anchor_classes='text-dark', $badge_classes='text-white bg-dark left-0', $nav_classes='d-none d-md-flex', $default_icon_classes='font-size-4', $icon_only=false, $id_base='sidebarNavToggler-' ) {
        ...
        if( ! empty( $links ) && is_array( $links ) ) {
            ...
            $end_key = key( $links );
            reset( $links );
            ?><ul class="header-icons-links nav<?php if( ! empty( $nav_classes ) ) echo esc_attr( ' ' . $nav_classes ); ?>"><?php
                foreach ( $links as $key => $link ) {
...
if ( ! function_exists( 'bookworm_site_header_icons_wc_cart_account_args' ) ) {
    function bookworm_site_header_icons_wc_cart_account_args( $links ) {
        if ( isset( $links['my_cart'] ) ) {
            $links['my_cart']['atts']['class'] = 'd-block nav-link text-dark';
            $links['my_cart']['item_class'] ='d-block';
        }

        if ( isset( $links['my_account'] ) ) {
            $links['my_cart']['atts']['class'] = 'd-block nav-link text-dark';
            $links['my_cart']['item_class'] ='d-block';
        }
