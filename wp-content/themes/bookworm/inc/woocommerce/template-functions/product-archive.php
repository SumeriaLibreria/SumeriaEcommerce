...
if ( ! function_exists( 'bookworm_wc_set_loop_shop_per_page' ) ) {
    ...
    function bookworm_wc_set_loop_shop_per_page( $per_page ) {
        ...
        elseif ( WC()->session->get( 'products_per_page' ) ) :
            $per_page = intval( WC()->session->get( 'products_per_page' ) );
        endif;
...
if ( ! function_exists( 'bookworm_wc_maybe_show_product_subcategories' ) ) {
    function bookworm_wc_maybe_show_product_subcategories() {
        wc_set_loop_prop( 'loop', 0 );
        $product_cat_columns = apply_filters( 'bookworm_product_cat_columns', 3 );
        ...
        if ( ! empty( $wc_sub_categories ) ) {
            ?><section class="section-product-categories">
                <div class="d-none">
                    <h2 class="section-title h3 mb-0"><?php echo sprintf( esc_html__( '%s Categories', 'bookworm' ), woocommerce_page_title( false ) ); ?></h2>
                </div>
                
                <ul class="list-unstyled loop-product-categories row row-cols-2 row-cols-lg-<?php echo esc_attr( $product_cat_columns ); ?>"><?php echo wp_kses_post(  $wc_sub_categories ); ?></ul></section><?php
        }
    }
}